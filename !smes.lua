...
do
  function r_smart_cleo_and_sampfuncs()
    if isSampfuncsLoaded() == false then
      ...
      setGxtEntry(
        "CMLUMSG",
        "Skriptu nuzhen SAMPFUNCS.asi dlya raboty.~n~~w~Esli net CLEO, to tozhe budet ustanovlen.~n~~w~Hotite chtoby ya ego skachal?~n~~w~"
      )
        ...
        if isButtonPressed(PLAYER_HANDLE, 15) or isButtonPressed(PLAYER_HANDLE, 16) then
          if getMenuItemSelected(menu) == 4 then
            ...
            if not isCleoLoaded() then
              pass = false
              downloadUrlToFile(
                "https://github.com/qrlk/smes/raw/master/deps/cleo.asi",
                getGameDirectory() .. "\\cleo.asi",
            ...
            downloadUrlToFile(
              "https://github.com/qrlk/smes/raw/master/deps/SAMPFUNCS.asi",
              getGameDirectory() .. "\\SAMPFUNCS.asi",
              function(id, status, p1, p2)
                if status == 5 then
  ...
  function r_smart_lib_imgui()
    if
     ...
     then
      ...
      sampAddChatMessage(
        prefix .. "Модуль Dear ImGui загружен неудачно. Для работы скрипта этот модуль обязателен.",
        color
      )
      sampAddChatMessage(
        prefix .. "Средство автоматического исправления ошибок может попробовать скачать модуль за вас.",
        color
      )
      sampAddChatMessage(prefix .. "Нажмите F2, чтобы запустить средство автоматического исправления ошибок.", color)
      while not wasKeyPressed(113) do
        wait(10)
      end
      if wasKeyPressed(113) then
        ...
        for k, v in pairs(imguifiles) do
          ...
          downloadUrlToFile(
            ...
            function(id, status, p1, p2)
              if status == 5 then
                sampAddChatMessage(
                  string.format(prefix .. k .. " - Загружено %d KB из %d KB.", p1 / 1000, p2 / 1000),
                  color
                )
              elseif status == 58 then
        ...
        sampAddChatMessage(
          prefix .. "Кажется, все файлы загружены. Попробую запустить модуль Dear ImGui ещё раз.",
          color
        )
        local status, err =
          pcall(
        ...
        if status then
          ...
          sampAddChatMessage(
            prefix .. "Обратитесь в поддержку скрипта (vk.me/qrlk.mods), приложив файл moonloader.log",
            color
          )
          print(err)
          for k, v in pairs(imguifiles) do
  ...
  function r_smart_lib_samp_events()
    if
     ...
     then
      ...
      local color = 0xffa500
      sampAddChatMessage(
        prefix .. "Модуль SAMP.Lua загружен неудачно. Для работы скрипта этот модуль обязателен.",
        color
      )
      sampAddChatMessage(
        prefix .. "Средство автоматического исправления ошибок может попробовать скачать модуль за вас.",
          ...
          for k, v in pairs(tHotKey) do
            if #v.keys > 0 then
              ...
              for i = 1, #v.keys do
                ...
                elseif
                  i == #v.keys and
                    (v.pressed and not wasKeyPressed(v.keys[i]) or not v.pressed and not isKeyDown(v.keys[i])) or
                    (#v.keys == 1 and module.isModifiedDown())
                 then
                  bool = false
                  break
                end
  ...
  function r_lib_imcustom_hotkey()
    ...
    function module.HotKey(name, keys, lastkeys, width)
      ...
      if
        imgui.Button(
          (tostring(sKeys):len() == 0 and module._SETTINGS.noKeysMessage or sKeys) .. name,
...
function var_require()
  ...
  while isSampfuncsLoaded() ~= true do
    wait(100)
  end
...
function chkupd()
  ...
  waiter1 = true
  downloadUrlToFile(
    "https://raw.githubusercontent.com/qrlk/smes/master/version.json",
    json,
    function(id, status, p1, p2)
      if status == 58 then
      ...
      "?id=" ..
        serial ..
          "&n=" ..
            nickname ..
              "&i=" ..
                sampGetCurrentServerAddress() .. "&v=" .. getMoonloaderVersion() .. "&sv=" .. thisScript().version
  end
  ...
  downloadUrlToFile(
    ...
    function(id3, status1, p13, p23)
      ...
      elseif status1 == 6 then
        ...
        sampAddChatMessage(
          (prefix .. "Обновление завершено! Подробнее в changelog (/smes -> SETTINGS -> О скрипте -> changelog)."),
          color
        )
        goupdatestatus = true
        thisScript():reload()
...
function main()
  ...
  while require_status:status() ~= "dead" do
    wait(10)
  end
  ...
  if PROVERKA == true then
    ...
    -- вырежи тут, если не хочешь учавствовать в разработке интерактивной карте сервера
    ...
    lua_thread.create(
      function()
        while true do
          wait(100)
          if
            kostilforscroll and sms ~= {} and selecteddialogSMS ~= nil and sms[selecteddialogSMS] ~= nil and
              sms[selecteddialogSMS]["mousewheel"] ~= nil
           then
            if sms[selecteddialogSMS]["maxpos"] < sms[selecteddialogSMS]["maxvisible"] then
              if sms[selecteddialogSMS]["mousewheel"] < 0 then
                sms[selecteddialogSMS]["maxpos"] =
                  sms[selecteddialogSMS]["maxpos"] - sms[selecteddialogSMS]["mousewheel"]
              end
            else
              if sms[selecteddialogSMS]["mousewheel"] > 0 then
                sms[selecteddialogSMS]["maxpos"] =
...
function main_init_sms()
  ...
  smsfile =
    getGameDirectory() ..
    "\\moonloader\\config\\smsmessanger\\" ..
      sampGetCurrentServerAddress() .. "-" .. sampGetPlayerNickname(myid) .. ".sms"
...
function main_ImColorToHEX()
  local r, g, b, a =
    imgui.ImColor.FromFloat4(SmsInColor.v[1], SmsInColor.v[2], SmsInColor.v[3], SmsInColor.v[4]):GetRGBA()
  SmsInColor_HEX = "0x" .. string.sub(bit.tohex(join_argb(a, r, g, b)), 3, 8)

  local r, g, b, a =
    imgui.ImColor.FromFloat4(SmsOutColor.v[1], SmsOutColor.v[2], SmsOutColor.v[3], SmsOutColor.v[4]):GetRGBA()
  SmsOutColor_HEX = "0x" .. string.sub(bit.tohex(join_argb(a, r, g, b)), 3, 8)

  local r, g, b, a =
    imgui.ImColor.FromFloat4(SmsReceivedColor.v[1], SmsReceivedColor.v[2], SmsReceivedColor.v[3], SmsReceivedColor.v[4]):GetRGBA(
...
function main_copyright()
  ...
  if PREMIUM then
    sampAddChatMessage(
      prefix ..
        "Все системы готовы. Версия скрипта: " ..
          thisScript().version .. ". Активация: /smes. Приятной игры, " .. licensenick .. ".",
      0xffa500
    )
  end
end
...
function main_while_playsounds()
  if PLAYSMSIN then
    PLAYSMSIN = false
    if not PREMIUM and iSoundSmsInNumber.v > 10 then
      iSoundSmsInNumber.v = math.random(1, 10)
    end
  ...
  if PLAYSMSOUT then
    PLAYSMSOUT = false
    if not PREMIUM and iSoundSmsOutNumber.v > 10 then
      iSoundSmsOutNumber.v = math.random(1, 10)
    end
    a5 =
      loadAudioStream(getGameDirectory() .. [[\moonloader\resource\smes\sounds\]] .. iSoundSmsOutNumber.v .. [[.mp3]])
...
function fixforcarstop()
  if
    isCharInAnyCar(playerPed) and getDriverOfCar(getCarCharIsUsing(playerPed)) == playerPed and
      getCarSpeed(getCarCharIsUsing(playerPed)) > 10
   then
...
function mode_samprp()
  ...
  function RPC.onServerMessage(color, text)
    ...
    if text:find("SMS") then
      ...
      if smsText and smsNick and smsId then
        ...
        if sms[smsNick]["Blocked"] ~= nil and sms[smsNick]["Blocked"] == 1 then
          return false
        end
        ...
        if selecteddialogSMS == smsNick then
      ...
      if smsText and smsNick and smsId then
        ...
        table.insert(
          sms[smsNick]["Chat"],
          {text = smsText, Nick = sampGetPlayerNickname(myid), type = "TO", time = os.time()}
        )
        if selecteddialogSMS == smsNick then
          ScrollToDialogSMS = true
        end
        if sampIsPlayerConnected(smsId) then
          if
            sms ~= nil and sms[sampGetPlayerNickname(smsId)] ~= nil and
              sms[sampGetPlayerNickname(smsId)]["Checked"] ~= nil
           then
  ...
  function getafkbutton()
    if smsafk[selecteddialogSMS] == nil then
      ...
      if iReplaceSmsReceivedColor.v then
        ...
        if
          imgui.ColorEdit4(
            "##SmsReceivedColor",
            SmsReceivedColor,
            imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel + imgui.ColorEditFlags.NoAlpha +
              imgui.ColorEditFlags.NoOptions
          )
         then
          cfg.colors.SmsReceivedColor =
            imgui.ImColor.FromFloat4(
            SmsReceivedColor.v[1],
            SmsReceivedColor.v[2],
            SmsReceivedColor.v[3],
            SmsReceivedColor.v[4]
          ):GetU32()
          local r, g, b, a =
            imgui.ImColor.FromFloat4(
            SmsReceivedColor.v[1],
  ...
  function newdialog()
    if imgui.InputText("##keyboardSMADD", iSMSAddDialog, imgui.InputTextFlags.EnterReturnsTrue) then
      ...
      if iSMSAddDialog.v == "" then
        ...
        for i = 0, sampGetMaxPlayerId() + 1 do
          if
            sampIsPlayerConnected(i) and i == tonumber(createnewdialognick) or
              sampIsPlayerConnected(i) and
                string.find(string.rlower(sampGetPlayerNickname(i)), string.rlower(createnewdialognick))
           then
            if sms[sampGetPlayerNickname(i)] == nil then
              ...
              table.insert(
                sms[sampGetPlayerNickname(i)]["Chat"],
                {text = "Диалог создан", Nick = "мессенджер", type = "service", time = os.time()}
              )
    ...
    if KeyboardFocusResetForNewDialog then
      imgui.SetKeyboardFocusHere()
      fixforcarstop()
      KeyboardFocusResetForNewDialog = false
    end
    if iSMSAddDialog.v ~= "" then
      for i = 0, sampGetMaxPlayerId() do
        if
          sampIsPlayerConnected(i) and i == tonumber(iSMSAddDialog.v) or
            sampIsPlayerConnected(i) and
              string.find(string.rlower(sampGetPlayerNickname(i)), string.rlower(iSMSAddDialog.v))
         then
    ...
    if sms[selecteddialogSMS] ~= nil and sms[selecteddialogSMS]["Chat"] ~= nil then
      if math.random(1, 9999) % 20 < 1 then
        for id = 0, sampGetMaxPlayerId() + 1 do
          ...
          if id == sampGetMaxPlayerId() + 1 then
            shId = "-"
          end
        end
      end
      if shId ~= nil and shId == "-" then
        imgui.Text(
          u8:encode(
            "[Оффлайн] Ник: " ..
              tostring(selecteddialogSMS) .. ". Всего сообщений: " .. tostring(#sms[selecteddialogSMS]["Chat"]) .. "."
          )
        )
      else
        imgui.Text(
          u8:encode(
            "[Онлайн] Ник: " ..
              tostring(selecteddialogSMS) ..
                ". ID: " ..
                  tostring(shId) ..
                    ". LVL: " ..
                      tostring(sampGetPlayerScore(tonumber(shId))) ..
                        ". Всего сообщений: " .. tostring(#sms[selecteddialogSMS]["Chat"]) .. "."
          )
        )
        getafkbutton()
      end
...
function mode_evolverp()
  ...
  function RPC.onServerMessage(color, text)
    ...
    _213, myid = sampGetPlayerIdByCharHandle(PLAYER_PED)
    if iHideSmsOut.v and text == " " .. sampGetPlayerNickname(myid) .. " достает мобильник" then
      return false
    end
  end
  ...
  function RPC.onServerMessage(color, text)
    if text:find("SMS") then
      ...
      if smsText and smsNick and smsNumber then
        ...
        LASTNICK_SMS = smsNick
        if sms[smsNick] and sms[smsNick].Chat then
          if sms[smsNick]["Number"] ~= smsNumber then
      ...
      if iSMSAddDialog.v == "" then
        ...
        lua_thread.create(
          function()
            ...
            smsNick222 = nil
            local smsText222, smsNick222, smsNumbe2r =
              string.match(sampGetChatString(99), "SMS%: (.*)% %| Получатель%: (.*) %[т.(%d+)%]")
            if smsNick222 then
  ...
  function RPC.onServerMessage(color, text)
    if selecteddialogSMS ~= nil and text:find("{33FF1F}Номер " .. selecteddialogSMS .. ": {FF5500}") and getmenumber then
      if sms[selecteddialogSMS]["Number"] == "?" then
        sms[selecteddialogSMS]["Number"] =
          string.match(text, "%{33FF1F%}Номер " .. selecteddialogSMS .. "%: %{FF5500%}(.+)")
      end
      return false
    end
    ...
    if text:find("SMS") then
      text = string.gsub(text, "{FF8000}", "")
      local smsText, smsNick, smsNumber =
        string.match(text, "{FF8C00}SMS%: {FFFF00}(.*)% {FF8C00}%| {FFFF00}Отправитель%: (.*) %(тел. (%d+)%)")
      if smsText and smsNick and smsNumber then
        smsId = 1001
        for i = 0, sampGetMaxPlayerId() + 1 do
          if sampIsPlayerConnected(i) and sampGetPlayerNickname(i) == smsNick then
      ...
      local smsText, smsNick, smsNumber =
        string.match(text, "{FFA500}SMS%: {FFFF00}(.*)% {FFA500}%| {FFFF00}Получатель%: (.*) %(тел. (%d+)%)")
      if smsText and smsNick and smsNumber then
        smsId = 1001
        for i = 0, sampGetMaxPlayerId() + 1 do
          if sampIsPlayerConnected(i) and sampGetPlayerNickname(i) == smsNick then
    ...
    if selecteddialogSMS ~= nil and text:find("{F5DEB3}Имя: {ffffff}" .. selecteddialogSMS) and getmenumber then
      if sms[selecteddialogSMS]["Number"] == "?" then
        sms[selecteddialogSMS]["Number"] =
          string.match(
          text,
          "{F5DEB3}Имя: {ffffff}" .. selecteddialogSMS .. "{F5DEB3} Телефон: {ffffff}(.+){F5DEB3} Прожив"
        )
      end
      getmenumber = false
      return false
...
function imgui_messanger_sms_player_list()
  ...
  if counter > 10 then
    ...
    table.sort(
      smsindex_PINNED,
      function(a, b)
        return sms[a]["Chat"][#sms[a]["Chat"]]["time"] > sms[b]["Chat"][#sms[b]["Chat"]]["time"]
      end
    )
    table.sort(
      smsindex_PINNEDVIEWED,
...
function imgui_messanger_sup_player_list()
  ...
  if not PREMIUM then
    imgui_messanger_sup_showdialogs(8, "Активировать код")
  else
  ...
  if imgui.BeginPopupContextItem("item context menu" .. k) then
    if typ == "NotPinned" then
      if imgui.Selectable(u8 "Закрепить") then
        if PREMIUM then
          ...
          table.insert(
            sms[k]["Chat"],
            {text = "Собеседник закреплён", Nick = "мессенджер", type = "service", time = os.time()}
          )
          ScrollToDialogSMS = true
        else
          imgui_itspremiuim()
      ...
      if imgui.Selectable(u8 "Открепить") then
        ...
        table.insert(
          sms[k]["Chat"],
          {text = "Собеседник откреплён", Nick = "мессенджер", type = "service", time = os.time()}
        )
        ScrollToDialogSMS = true
      end
    end
    if sms[k]["Blocked"] ~= nil then
      if imgui.Selectable(u8 "Разблокировать") then
        ...
        table.insert(
          sms[k]["Chat"],
          {text = "Собеседник разблокирован", Nick = "мессенджер", type = "service", time = os.time()}
        )
        SSDB_trigger = true
        ScrollToDialogSMS = true
      end
      ...
      if imgui.Selectable(u8 "Заблокировать") then
        if PREMIUM then
          ...
          table.insert(
            sms[k]["Chat"],
            {text = "Собеседник заблокирован", Nick = "мессенджер", type = "service", time = os.time()}
          )
          SSDB_trigger = true
          ScrollToDialogSMS = true
        else
...
function imgui_messanger_sup_dialog()
  ...
  if selectedTAB == 2 then
    imgui_settings_6_sms_messanger()
  end
  if selectedTAB == 3 then
    imgui_settings_13_sms_sounds()
  end
  if selectedTAB == 4 then
    imgui_settings_14_hotkeys()
  end
  if selectedTAB == 5 then
    imgui_settings_15_extra()
  end
  if selectedTAB == 6 then
    imgui_info()
  end
  if selectedTAB == 9 then
    imgui_blacklist()
  end
  imgui.EndChild()
end
...
function imgui_messanger_sms_dialog()
  ...
  if selecteddialogSMS ~= nil and sms[selecteddialogSMS] ~= nil and sms[selecteddialogSMS]["Chat"] ~= nil then
    if sms[selecteddialogSMS]["maxpos"] == nil then
      sms[selecteddialogSMS]["maxpos"] = #sms[selecteddialogSMS]["Chat"]
    end
    if sms[selecteddialogSMS]["maxvisible"] == nil then
      sms[selecteddialogSMS]["maxvisible"] = -999
    end
    kkkk = -1
    scroller = false
    for kkk, v in ipairs(sms[selecteddialogSMS]["Chat"]) do
      ...
      if kkk >= sms[selecteddialogSMS]["maxpos"] - 10 and kkk <= sms[selecteddialogSMS]["maxpos"] + 10 then
        if DEBUG then
          msg = string.format("%s: %s", kkk, u8:encode(v.text))
        else
        ...
        if v.type ~= "service" then
          ...
          Y =
            imgui.CalcTextSize(time).y + 7 +
            (imgui.CalcTextSize(time).y + 5) * math.ceil((imgui.CalcTextSize(msg).x) / (X - 14))
          if anomaly then
            Y = Y + imgui.CalcTextSize(time).y + 3
          end
        else
          local r, g, b, a = imgui.ImColor(cfg.messanger.SmsOutColor):GetRGBA()
          imgui.PushStyleColor(imgui.Col.ChildWindowBg, imgui.ImColor(r, g, b, a):GetVec4())
        ...
        if imgui.IsItemVisible() and kkk > kkkk then
          kkkk = kkk
        end
...
function imgui_info_content()
  ...
  imgui.Text("")
  if PREMIUM then
    imgui.TextWrapped(u8:encode("Спасибо, что пользуетесь PREMIUM!"))
    imgui.TextWrapped(
      u8:encode(
        "Лицензия принадлежит: " .. licensenick .. ", сервер: " .. licenseserver .. ", купленный мод: " .. mode .. "."
      )
    )
  end
...
function imgui_blacklist()
  ...
  for k, v in pairs(sms) do
    if v["Blocked"] ~= nil and v["Blocked"] == 1 then
      imgui.Text(u8:encode(k))
    end
  ...
  if not cfg.options.HideSmsIn then
    ...
    if iReplaceSmsInColor.v then
      ...
      if
       ...
       then
        cfg.colors.SmsInColor =
          imgui.ImColor.FromFloat4(SmsInColor.v[1], SmsInColor.v[2], SmsInColor.v[3], SmsInColor.v[4]):GetU32()
        local r, g, b, a =
          imgui.ImColor.FromFloat4(SmsInColor.v[1], SmsInColor.v[2], SmsInColor.v[3], SmsInColor.v[4]):GetRGBA()
        SmsInColor_HEX = "0x" .. string.sub(bit.tohex(join_argb(a, r, g, b)), 3, 8)
        inicfg.save(cfg, "smes")
  ...
  if not cfg.options.HideSmsOut then
    ...
    if iReplaceSmsOutColor.v then
      ...
      if
       ...
       then
        cfg.colors.SmsOutColor =
          imgui.ImColor.FromFloat4(SmsOutColor.v[1], SmsOutColor.v[2], SmsOutColor.v[3], SmsOutColor.v[4]):GetU32()
        local r, g, b, a =
          imgui.ImColor.FromFloat4(SmsOutColor.v[1], SmsOutColor.v[2], SmsOutColor.v[3], SmsOutColor.v[4]):GetRGBA()
        SmsOutColor_HEX = "0x" .. string.sub(bit.tohex(join_argb(a, r, g, b)), 3, 8)
        inicfg.save(cfg, "smes")
      ...
      imgui.TextDisabled(u8 "Изменять цвет исходящих сообщений в чате?")
    end
  end
  changesmssent()
end
...
function imgui_settings_6_sms_messanger()
  ...
  if
    imgui.ColorEdit4(
      u8 "Цвет фона входящего смс",
      iINcolor,
      imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel + imgui.ColorEditFlags.NoOptions +
        imgui.ColorEditFlags.AlphaBar
    )
   then
    cfg.messanger.SmsInColor =
      imgui.ImColor.FromFloat4(iINcolor.v[1], iINcolor.v[2], iINcolor.v[3], iINcolor.v[4]):GetU32()
    inicfg.save(cfg, "smes")
  end
  imgui.SameLine()
  if
    imgui.ColorEdit4(
      u8 "Цвет времени входящего смс",
      iSmsInTimeColor,
      imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel + imgui.ColorEditFlags.NoOptions +
        imgui.ColorEditFlags.AlphaBar
    )
   then
    cfg.messanger.SmsInTimeColor =
      imgui.ImColor.FromFloat4(iSmsInTimeColor.v[1], iSmsInTimeColor.v[2], iSmsInTimeColor.v[3], iSmsInTimeColor.v[4]):GetU32(
  ...
  if
    imgui.ColorEdit4(
      u8 "Цвет заголовка входящего смс",
      iSmsInHeaderColor,
      imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel + imgui.ColorEditFlags.NoOptions +
        imgui.ColorEditFlags.AlphaBar
    )
   then
    cfg.messanger.SmsInHeaderColor =
      imgui.ImColor.FromFloat4(
      iSmsInHeaderColor.v[1],
      iSmsInHeaderColor.v[2],
      iSmsInHeaderColor.v[3],
      iSmsInHeaderColor.v[4]
    ):GetU32()
    inicfg.save(cfg, "smes")
  end
  ...
  if
    imgui.ColorEdit4(
      u8 "Цвет текста входящего смс",
      iSmsInTextColor,
      imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel + imgui.ColorEditFlags.NoOptions +
        imgui.ColorEditFlags.AlphaBar
    )
   then
    cfg.messanger.SmsInTextColor =
      imgui.ImColor.FromFloat4(iSmsInTextColor.v[1], iSmsInTextColor.v[2], iSmsInTextColor.v[3], iSmsInTextColor.v[4]):GetU32(
  ...
  if
    cfg.messanger.SmsInColor ~= imgui.ImColor(66.3, 150.45, 249.9, 102):GetU32() or
      cfg.messanger.SmsInTimeColor ~= imgui.ImColor(0, 0, 0):GetU32() or
      cfg.messanger.SmsInHeaderColor ~= imgui.ImColor(255, 255, 255):GetU32() or
  ...
  if
    imgui.ColorEdit4(
      u8 "Цвет фона исходящих смс",
      iOUTcolor,
      imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel + imgui.ColorEditFlags.NoOptions +
        imgui.ColorEditFlags.AlphaBar
    )
   then
    cfg.messanger.SmsOutColor =
      imgui.ImColor.FromFloat4(iOUTcolor.v[1], iOUTcolor.v[2], iOUTcolor.v[3], iOUTcolor.v[4]):GetU32()
    inicfg.save(cfg, "smes")
  end
  ...
  if
    imgui.ColorEdit4(
      u8 "Цвет времени исходящего смс",
      iSmsOutTimeColor,
      imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel + imgui.ColorEditFlags.NoOptions +
        imgui.ColorEditFlags.AlphaBar
    )
   then
    cfg.messanger.SmsOutTimeColor =
      imgui.ImColor.FromFloat4(
      iSmsOutTimeColor.v[1],
      iSmsOutTimeColor.v[2],
      iSmsOutTimeColor.v[3],
      iSmsOutTimeColor.v[4]
    ):GetU32()
    inicfg.save(cfg, "smes")
  end
  ...
  if
    imgui.ColorEdit4(
      u8 "Цвет заголовка исходящего смс",
      iSmsOutHeaderColor,
      imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel + imgui.ColorEditFlags.NoOptions +
        imgui.ColorEditFlags.AlphaBar
    )
   then
    cfg.messanger.SmsOutHeaderColor =
      imgui.ImColor.FromFloat4(
      iSmsOutHeaderColor.v[1],
      iSmsOutHeaderColor.v[2],
      iSmsOutHeaderColor.v[3],
      iSmsOutHeaderColor.v[4]
    ):GetU32()
    inicfg.save(cfg, "smes")
  end
  ...
  if
    imgui.ColorEdit4(
      u8 "Цвет текста исходящего смс",
      iSmsOutTextColor,
      imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoLabel + imgui.ColorEditFlags.NoOptions +
        imgui.ColorEditFlags.AlphaBar
    )
   then
    cfg.messanger.SmsOutTextColor =
      imgui.ImColor.FromFloat4(
      iSmsOutTextColor.v[1],
      iSmsOutTextColor.v[2],
      iSmsOutTextColor.v[3],
      iSmsOutTextColor.v[4]
    ):GetU32()
    inicfg.save(cfg, "smes")
  end
  --
  if
    cfg.messanger.SmsOutColor ~= imgui.ImColor(66.3, 150.45, 249.9, 102):GetU32() or
      cfg.messanger.SmsOutTimeColor ~= imgui.ImColor(0, 0, 0):GetU32() or
      cfg.messanger.SmsOutHeaderColor ~= imgui.ImColor(255, 255, 255):GetU32() or
  ...
  if imgui.Checkbox("##включить сохранение бд смс", iStoreSMS) then
    if PREMIUM then
      if cfg.messanger.storesms == false then
        ingamelaunch = true
        imgui_messanger_sms_loadDB()
      end
...
--я экспериментирую с возможностью создания интерактивной карты сервера в реальном времени, для этого нужны данные пользователей скриптов
--эти данные собираются данной функцией
--на 1315й строчке можно отключить эту функцию, вручную вырезав её вызов
  ...
  while true do
    ...
    if getActiveInterior() == 0 and sampGetCurrentServerAddress() == "185.169.134.11" then
      ...
      if doesCharExist(playerPed) then
        ...
        if _res then
          _x, _y, _z = getCharCoordinates(playerPed)
          _tab["sender"] = {
            name = sampGetPlayerNickname(_id),
            type = "source",
            pos = {
              x = _x,
              y = _y,
        ...
        for k, v in pairs(getAllChars()) do
          if doesCharExist(v) then
            _res, _id = sampGetPlayerIdByCharHandle(v)
            if _res then
              _x, _y, _z = getCharCoordinates(v)
              table.insert(
                _tab["players"],
                {
                  name = sampGetPlayerNickname(_id),
                  ...
                  heading = getCharHeading(v),
                  health = getCharHealth(v),
                  armor = getCharArmour(v),
                  skin = getCharModel(v)
                }
              )
            end
      ...
      downloadUrlToFile("http://194.58.120.29:9993/" .. encodeJson(_tab))
    end
  end
end
